...
目前，我们提供了两种方式来安装JD Fusion CLI。第一种，Dokcer容器，第二种RPM包。

### 以Docker容器的方式安装运行

使用本文档中提供的CLI运行环境Docker镜像[jdfusion/fusion-cli](https://hub.docker.com/r/jdfusion/fusion-cli/)。这需要你本地的电脑上安装了Docker程序，关于Docker相关的文档，请参考[Docker官方文档](https://www.docker.com/)

在使用Docker拉取镜像，创建容器之前，需要准备以下信息：
>* JD Fusion平台的用户AK
>* JD Fusion平台的用户SK
...
# jdfusion/fusion-cli list clouds
```
将`$AK`、`$SK`替换为你自己在JD Fusion平台上AK、SK即可。若不知如何获取，请查看[JD Fusion平台用户认证](https://github.com/jdfusion/jdfusion.github.io)

示例中，
`-e` 为设置程序运行所需要的环境变量
`jdfusion/fusion-cli`为官方提供的JD Fusion/cli运行环境镜像
`list clouds`为实际运行的指令，用于展示用户注册的云信息列表

#### 命令行使用环境变量文件
命令如下：
```bash
# docker run -ti --env-file env.list jdfusion/fusion-cli list clouds
```
env.list的文件内容格式为：
```properties
...
使用本文档中提供的[fusion-cli.rpm](http://jd-fusion-pub-storage.oss.cn-north-1.jcloudcs.com/releases/cli/rpm/fusion-cli-0.1.1~RELEASE.x86_64.rpm)包，安装jd fusion-cli。目前，仅提供64位版本。
下载RPM包到你的Linux系统中，在终端中运行命令：
```bash
rpm -i fusion-cli-*.x86_64.rpm
```
安装完成后，需要进行一些配置才可以进行使用。在当前用户的home目录下，创建一个".fusion-cloud"文件，在文件中，以property的方式配置ak，sk等信息。
示例如下：
```bash
cd ~
touch .fusion-cloud
echo "x.jdcloud.fusion.ak=$AK" >> .fusion-cloud
echo "x.jdcloud.fusion.sk=$SK" >> .fusion-cloud
echo "x.jdcloud.fusion.region=cn-north-1a" >> .fusion-cloud
```
将`$AK`、`$SK`替换为你自己在JD Fusion平台上AK、SK即可。若不知如何获取，请查看[JD Fusion平台用户认证](https://github.com/jdfusion/jdfusion.github.io)
